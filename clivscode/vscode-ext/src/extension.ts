...
import { TecspacsProvider } from './providers/tecspacsProvider';
import { Logger } from './utils/logger';

export function activate(context: vscode.ExtensionContext) {
    Logger.info('Tecspacs extension is now active!');

    // Register commands
    const commands = new TecspacsCommands(context);
    
    // Register the tree data provider for the sidebar
    const tecspacsProvider = new TecspacsProvider(commands);
    ...
    const insertSnippetCommand = vscode.commands.registerCommand('tecspacs.insertSnippet', async (snippetId: string) => {
        await commands.insertSnippetById(snippetId);
    });

    // View package details command for tree view items
    const viewPackageCommand = vscode.commands.registerCommand('tecspacs.viewPackage', async (packageId: string) => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tecspacs.updateStatusBar', updateStatusBar)
    );
...
export function deactivate() {
    Logger.info('Tecspacs extension is now deactivated!');
} 