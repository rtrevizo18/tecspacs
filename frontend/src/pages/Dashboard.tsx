...
const Dashboard: React.FC = () => {
  ...
  const getFilteredItems = () => {
    const tecItems = allTECs.map(tec => ({ ...tec, itemType: 'TEC' as const }));
    const pacItems = allPACs.map(pac => ({ ...pac, itemType: 'PAC' as const }));
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-4 py-2 text-sm font-bold border-2 border-pen-black rounded transition-colors ${
                    filter === 'all' 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-6">
            {filteredItems.length > 0 ? (
              filteredItems.map((item) => {
                const user = getUserById(item.author) || { name: 'Unknown', auth0Id: item.author };
                return (
                  <ItemCard
